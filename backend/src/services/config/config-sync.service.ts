      ...
      if (feed.outputs.recap?.enabled && feed.outputs.recap.schedule) {
        ...
        try {
          ...
          if (existingJob) {
            ...
            logger.info(
              `Updated recap job for feed ${feed.id} with schedule ${feed.outputs.recap.schedule}, next run at ${nextRunAt}`,
            );
          } else {
            // Create new job
            ...
            logger.info(
              `Created recap job for feed ${feed.id} with schedule ${feed.outputs.recap.schedule}, next run at ${nextRunAt}`,
            );
          }
        } catch (error) {
