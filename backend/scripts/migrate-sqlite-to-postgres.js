...
const PG_CONNECTION_STRING = process.env.PG_CONNECTION_STRING || 'postgresql://postgres:postgres@localhost:5432/curatedotfun';
  ...
  try {
    ...
    for (const feed of feeds) {
      await pgClient.query(
        ...
        [feed.id, feed.name, feed.description, feed.created_at, feed.updated_at]
      );
    }
    ...
    const submissionFeeds = sqlite.prepare('SELECT * FROM submission_feeds').all();
    let migratedSubmissionFeeds = 0;
    let skippedSubmissionFeeds = 0;
    
    // First, get all valid submission IDs from PostgreSQL
    const submissionResult = await pgClient.query('SELECT tweet_id FROM submissions');
    const validSubmissionIds = new Set(submissionResult.rows.map(row => row.tweet_id));
    ...
    for (const sf of submissionFeeds) {
      ...
      if (validSubmissionIds.has(sf.submission_id) && validFeedIds.has(sf.feed_id)) {
        try {
          await pgClient.query(
          ...
          console.error(`Error migrating submission_feed (${sf.submission_id}, ${sf.feed_id}):`, error.message);
    ...
    console.log(`Migrated ${migratedSubmissionFeeds} submission_feeds (skipped ${skippedSubmissionFeeds})`);

    // Migrate moderation_history
    console.log('Migrating moderation_history...');
    const moderationHistory = sqlite.prepare('SELECT * FROM moderation_history').all();
    ...
    for (const mh of moderationHistory) {
      ...
      if (validSubmissionIds.has(mh.tweet_id) && validFeedIds.has(mh.feed_id)) {
        try {
          ...
          console.error(`Error migrating moderation_history for tweet ${mh.tweet_id}:`, error.message);
    ...
    console.log(`Migrated ${migratedModerationHistory} moderation_history records (skipped ${skippedModerationHistory})`);

    // Migrate submission_counts
    console.log('Migrating submission_counts...');
    const submissionCounts = sqlite.prepare('SELECT * FROM submission_counts').all();
    ...
    const twitterCookies = sqlite.prepare('SELECT * FROM twitter_cookies').all();
